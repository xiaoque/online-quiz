...
public class FraudulentActivityNotifications {
     ...
     * @date 2021.06.13
     *
     * Tried to solve this using counting sort approach but got time out problem for
     * the test cases
     * so I changed to a tricky one by using priority queue.
     * the prevQ stores the first half of numbers in desc order (when d is odd, it
     * stores the extra one)
     * the lastQ stores another half of the numbers in asc order
     * so 2 * mid will always be sum of the top of the two queues when d is even,
     * and 2 * top of prevQ when d is odd
     */
    public static int activityNotifications(List<Integer> expenditure, int d) {
        ...
        for (int i = d; i < expenditure.size(); ++i) {
            ...
            if (prevQ.contains(expenditure.get(i - d)))
                ...
                lastQ.remove(expenditure.get(i - d));
